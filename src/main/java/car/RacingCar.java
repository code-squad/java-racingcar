...
public class RacingCar {
	...
	public static ArrayList<Integer> startRace(ArrayList<Car> car) {
		ArrayList<Integer> carLocation = new ArrayList<Integer> (car.size());
		for (int i = 0; i < car.size(); i++) {
			Random randomNum = new Random();		//random 변수를 만들어준다.
			int moveDice = randomNum.nextInt(11);		//만들어준 변수에 0~10 사이의 랜덤 수를 입력해준다.
			
			carLocation.add(i, car.get(i).move(moveDice));
		}
		return carLocation;
	}
