...
// 마지막에 콤마 제거하는 방법
	public static void main(String[] args) {
		...
		String[] nameArr = carName(scanner);   // 경기에 참가할 참가자의 이름을 입력한다.
		Car[] cars = cars(nameArr);   // 각 참가자(인스턴스)를 생성한다.
		
		System.out.println("시도할 회수는 몇회인가요?");
		int tries = tryNum(scanner);   // 총 몇번 시도할 것인지 입력한다.
		run(cars, tries, random);   // 경기를 시작한다.
	
		Vector winners = winners(cars);   // 우승자를 가린다.
		resultView(winners);   // 우승자를 출력한다.
	}
	...
	static Vector winners(Car[] cars ) {   // 우승자를 가리는 메서드
		...
		for (int i = 0; i < cars.length; i++) {   // 우승자가 몇명인지 가린다.
			if (max == cars[i].counter)
				winners.add(cars[i]);
		}
	...
	static void resultView(Vector winners) {   // 경기결과를 알려주는 메서드
		...
		for (int i = 0; i < winners.size(); i++) {  // 문제생김 배열의 수를 어떻게 할 것인가.
