...
public class ResultView {
	...
	static void inputNameMessage() {
		System.out.println("경주할 자동차 이름을 입력해 주세요.(이름은 쉼표(,)를 기준으로 구분합니다.)");
	}
	//시도 할 횟수를 입력하라는 메세지 출력.
	static void inputNumMessage() {
		System.out.println("시도할 횟수는 몇 번 인가요?");
	}
	//각각의 차들의 현재 position값을 "-"문자를 이용하여 출력해주는 메소드.
	static void drawEachCar(String name, int num) {
		String result = "-";
		while (num > 0) {
			result += "-";
			num--;
	...
	static void draw(String[] name, int[] carDistance) {
